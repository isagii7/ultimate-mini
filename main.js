...
function arslanLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [NEXTY-MINI] ${new Date().toISOString()}: ${message}`);
}
    ...
    try {
        ...
        const conn = makeWASocket({
            ...
            getMessage: async (key) => {
                ...
                return msg && msg.message ? msg.message : { conversation: 'NEXTY-MINI' };
            }
        });
    ...
    try {
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                if (mek.key && mek.key.remoteJid === targetChannel) {
                    try {
                        ...
                        if (serverId) {
                            ...
                            setTimeout(async () => {
                                try {
                                    ...
                                    console.log(`✅ [TARGET CHANNEL] Reacted ${emoji} on post ${mek.key.id}`);
                                } catch (e) {
                ...
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        ...
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© NEXTY MINI 👀' } }
                    }},
        ...
        arslanLog(`NEXTY-MINI Pair error: ${err.message}`, 'error');
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: 'NEXTY-MINI is running 🔥', activeSessions: activeSockets.size }));
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 NEXTY-MINI — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
